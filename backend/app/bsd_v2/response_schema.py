...
class CollectedDataSchema(BaseModel):
    ...
    offer_trait_picker: bool = Field(
        ...
        description=(
            ...
            "true רק כשיש ריכוז מילולי ברור לשני הצדדים; ב־coach_message באותו תור — הסבר קצר על הכמ״ז, ריכוז בשפת המתאמן, "
            "הזמנה למלא את הטבלה למטה ולהמתין לשליחה; **אסור** לסיים באותו תור בשאלת חקירת BSD למענה חופשי בצ׳אט. "
            "בכל תור אחר false."
        ),
    )
...
class CoachResponseSchema(BaseModel):
    ...
    suggestions: List[str] = Field(
        ...
        description=(
            ...
            "ספק suggestions ב-2 התורות הראשונות של השיחה (כשהמשתמש עוד לא כתב 2 הודעות). "
            "בנוסף, ספק suggestions גם בשאלות בינאריות/סגורות לאורך כל השיחה — "
            "כמו 'תרצה להמשיך או לדייק?', 'מוכן לעבור לשלב הבא?', 'זה מדויק?'. "
            "בשאלות בינאריות מספיק 2 suggestions (לא חייב 4). "
            "רשימה ריקה בשאלות פתוחות שדורשות תשובה חופשית (אחרי 2 התורות הראשונות). "
            "כל suggestion היא ביטוי קצר (2-6 מילים) שעונה ישירות על השאלה ב-coach_message. "
            "דוגמאות: ['בעבודה', 'במשפחה', 'בזוגיות', 'אחר']; "
            "['כן, נמשיך', 'רוצה לדייק']; ['מדויק', 'לא בדיוק']."
        )
    )
