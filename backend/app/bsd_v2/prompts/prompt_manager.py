...
STAGE_GATES_HE: Dict[str, str] = {
    ...
    "S12": "**Gate (S12→S13):** חקר מקור ואז חקר טבע; ב־`forces` **6 תכונות ב־`source`**, **6 ב־`nature`** (כרטיס כמ״ז לפי השיטה); **הפריט הראשון** בכל רשימה = התכונה המובילה. אם אין 6+6 — השלם בשיחה או הישאר ב־S12 עד הסכמה מפורשת לגרסה מקוצרת.",
...
STAGE_GATES_EN: Dict[str, str] = {
    ...
    "S12": "**Gate (S12→S13):** Source explored, then nature; in `forces` **6 traits in `source`**, **6 in `nature`** (KaMaZ card per BSD methodology); **first item** in each list = leading trait. If not 6+6 — complete in dialogue or stay in S12 until explicit consent to a shorter card.",
