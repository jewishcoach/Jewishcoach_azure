...
export const RemindersManager = ({ variant = 'dark' }: RemindersManagerProps) => {
  ...
  const inputCls = isLight ? 'border-gray-200 bg-white text-[#1E293B] placeholder-gray-400 focus:ring-[#0EA5E9]/30' : 'border-white/10 bg-white/[0.04] text-[#F5F5F0] placeholder-[#F5F5F0]/40 focus:ring-[#FCF6BA]/30';
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <Bell className={`w-5 h-5 ${isLight ? 'text-[#0EA5E9]' : 'text-[#FCF6BA]'}`} />
          <h3 className={`text-lg font-bold ${textCls}`}>{t('reminders.title')}</h3>
        </div>
        <button
          onClick={() => setShowAddForm(!showAddForm)}
          className="flex items-center gap-2 px-4 py-2 rounded-lg transition-colors"
          style={isLight ? { background: '#0EA5E9', color: 'white' } : { background: 'linear-gradient(45deg, #BF953F, #FCF6BA)', color: '#020617' }}
        >
        ...
        {showAddForm && (
          ...
          >
            <div className="space-y-4">
                      ...
                      <button
                        ...
                        className={`w-10 h-10 rounded-full transition-colors ${
                          formData.repeat_days.includes(index)
                            ? isLight ? 'bg-[#0EA5E9]/30 text-[#0EA5E9]' : 'bg-[#FCF6BA]/30 text-[#FCF6BA]'
              ...
              <div className="flex gap-2">
                <button type="submit" className="flex items-center gap-2 px-6 py-2 rounded-lg transition-colors" style={isLight ? { background: '#0EA5E9', color: 'white' } : { background: 'linear-gradient(45deg, #BF953F, #FCF6BA)', color: '#020617' }}>
            ...
            <motion.div key={reminder.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`rounded-xl p-4 border ${cardCls}`}>
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Bell className={`w-4 h-4 ${isLight ? 'text-[#0EA5E9]' : 'text-[#FCF6BA]'}`} />
                  ...
                  <div className={`flex items-center gap-4 mt-2 text-sm ${mutedCls}`}>
                    ...
                    <span className={`px-2 py-1 rounded text-xs ${isLight ? 'bg-[#0EA5E9]/15 text-[#0EA5E9]' : 'bg-[#FCF6BA]/15 text-[#FCF6BA]'}`}>
                      {getRepeatLabel(reminder.repeat_type, t)}
                    </span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(reminder)} className={`p-2 ${mutedCls} hover:opacity-100 transition-colors ${isLight ? 'hover:text-[#0EA5E9]' : 'hover:text-[#FCF6BA]'}`}>
